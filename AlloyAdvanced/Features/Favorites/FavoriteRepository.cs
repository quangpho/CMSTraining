...
namespace AlloyAdvanced.Features.Favorites
{
    public class FavoriteRepository
    {
        ...
        public static void Save(Favorite fav)
        {
            ...
            {
                throw new NullReferenceException("Unable to add favorite without user name");
            }
            store.Save(fav);
        }
        ...
        public static Favorite GetFavorite(ContentReference contentReference, string userName)
        {
            return store.Items<Favorite>()
            .Where(fav => (fav.UserName == userName) &&
